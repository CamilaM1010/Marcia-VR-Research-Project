TL;DR: The Stimulus can be used to play an animation and/or an audio clip. It can be triggered by calling the TriggerStimulus() function somewhere in code or by attaching a StimulusActionTrigger and choosing an action. A StimulusSequence plays a series of Stimuli in order, with an optional delay after each one, accessible via the same triggering methods as each stimuli. As well, both Stimuli and StimulusSequences can have a button reference that will automatically change the UI text for each one to be easily used, assuming the buttons are using the "Stimulus Button" prefab.
...
3. Set the step's Stimulus reference to the first Stimulus you wish to be triggered, as well as the delay (in seconds) to occur after the trigger.
